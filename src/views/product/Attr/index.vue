<template>
  <div>
    <el-card style="margin:20px 0">
      <CategorySelect @getCategoryId="getCategoryId" :isShowTable="isShowTable" />
    </el-card>
    <el-card>
      ...
      <div v-show="!isShowTable">
        ...
        <el-table style="width: 100%;margin:20px 0" border :data="attrInfo.attrValueList">
          ...
          <el-table-column prop="prop" label="属性值名称" width="width">
            <template slot-scope="{row,$index}">
              <!-- span与input来回切换 -->
              <el-input v-model="row.valueName" placeholder="请输入属性值名称" size="mini" v-if="row.flag" @blur="toLook(row)" @keyup.enter="toLook(row)" :ref="$index"></el-input>
...
export default {
  ...
  data() {
    return {
      ...
      attrInfo: {
        attrName: "", // 属性名
        attrValueList: [ // 属性值，属性值可以有多个因此用数组，每一个属性值都是一个对象需要attrId，valueName
        ],
        categoryId: 0, // 三级分类的id
        categoryLevel: 0, // 服务器也需要区分是几级id
      }
    }
  },
  methods: {
    ...
    // 当用户确定三级分类的数据的时候，向服务器发请求获取相应的属性进行展示
    async getAttrList() {
      const {
        category1Id,
        category2Id,
